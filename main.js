const myself = {
       myName: "Fahmida Yeasmin",
       age   :"22",
       study :"Diploma In Computer Engineering",
       studyCenter:"ISTT",
       metarialStatus: "Married",
    family:{
        parents:{
            father:"Md.Alauddin",
            age   :55,
            always_say:"Meye to na jeno akta agun Aar gola amr🙂 Atto rag ...Afterall meye to amr e 😂",
            mother:"Mrs.Nazma Yeasmin",
            age   :40,
            always_say:"Amr Brithe moni ja bole ta kore dekhay😊😊",
        },
        siblings :{
            sister:{
            name :"Farhana Yeasmin",
            age  :35,
            myMsg:"World's best sister u r! U dont't know how much i love u ❤️❤️.",
        },
    
            elderBrother:{
                name :"Mazharul Islam",
                age  :28,
                myMsg:"World's best protective brother u r !" ,
            },
            youngerBrother:{
                name :"Monjurul Islam",
                age  :19,
                myMsg:"R maris na amare  vai😥 iktu  to respect kor" ,
            },
    
        },
            
    },
    my_Legends_Friends:{
        Rameem:{
            name :"Mahmood Hossain Rameem",
            age  : 19,
           playCharacter:"Everything ok when fahmida say yes! Overall Alavola vai amr  !",
          always_say:"R a parba tumi❤️...vhoy nai ami achi✌️😊😊 "
    },
    Rasel:{
        name :"Jahidul Islam Rasel",
        age  : 21,
       playCharacter:"Expert of khowadaowa",
      always_say:" khida lagche vai😥Fahmida cholo Baten re  chuna die😂 "
},
     Baten:{
            name :"Abdul Baten",
            age  : 21,
           playCharacter:"Always chuna khay😂But fahmidar vhoy a kichu bole o na ",
          always_say:"taka nai😥(gujobe kan diben na kau😂) "
    },
    Pathan:{
      name :"Pabel Pathan",
      age  : 20,
      playCharacter:"Best photographer in Legend group",
      always_say:" Ato pora lage naki ? Amni e A+✌️"
    },
    Mahabul:{
            name :"Mahabul Alam",
            age  : 19,
            playCharacter:"Chupchap...kintu Erin ful hole bapper ta different😜",
            always_say:"Apu ami kichu komu na🙈 "
    },
    Sayma:{
        name :"Maysha Siddiki",
            age  : 20,
            playCharacter:"Sentimental Girl",
            always_say:"Fahmida tomar moto akta borka banamu😂😂"


        }

    },

}


console.log(myself.Rameem);